export class MotivationSystem {
    ...
    async startWeeklyChallenge(challenge) {
        const weekId = this.getWeekId();
        await this.db.get('challenges')
            .get(weekId)
            .put({
                ...challenge,
        ...
        return new Promise((resolve) => {
            ...
            this.db.get('challenges')
                ...
                .once((challenge) => {
                    ...
                    this.calculateWeeklyProgress(challenge)
                        .then((progress) => {
                            resolve({
                                challenge,
                                currentHours: progress,
                                completed: progress >= challenge.targetHours
                            });
                        });
                });
        });
    ...
    async calculateWeeklyProgress(challenge) {
        return new Promise((resolve) => {
            let totalHours = 0;
            ...
            this.db.get('studyTime')
                .map((data) => {
                    if (!data || data.subject !== challenge.subject) return;
                    if (new Date(data.date) >= weekStart) {
                        totalHours += data.duration / 3600;
                    }
                });

            setTimeout(() => resolve(totalHours), 100);
        });
    }
